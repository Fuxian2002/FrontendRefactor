...
const initScenarioGraph = (proj: any) => {
  ...
  proj.scenarioGraphList.forEach((sg: any) => {
    if (sg.intNodeList && sg.intNodeList.length > 0) return; // Already populated
...
.group_5 {
    ...
    gap: 1.5vw; /* Add gap between buttons */
}
...
.text-wrapper_18, .text-wrapper_19 {
  ...
  height: 2.8vw;
  display: flex;
  flex-direction: row; /* Ensure row layout for proper vertical alignment */
  justify-content: center;
  align-items: center;
  border-radius: 6px;
  cursor: pointer;
...
.text_30, .text_31 {
  ...
  padding-bottom: 0.2vw; /* Manually adjust baseline to center visually */
}
