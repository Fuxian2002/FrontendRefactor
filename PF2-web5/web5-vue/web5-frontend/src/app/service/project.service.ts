...
export class ProjectService {
	...
	getReference(project: Project): Observable<RequirementPhenomenon[]>{
		const url = `http://localhost:7086/project/getReference`;
	...
	checkCorrectness(project: Project): Observable<Error[]> {
		const url = `http://localhost:7086/project/checkCorrectness`;
		return this.http.post<Error[]>(url, project, this.httpOptions);
	}

	checkCorrectContext(project: Project): Observable<Error[]> {
		const url = `http://localhost:7086/project/checkCorrectContext`;
		return this.http.post<Error[]>(url, project, this.httpOptions);
	}

	checkCorrectProblem(project: Project): Observable<Error[]> {
		const url = `http://localhost:7086/project/checkCorrectProblem`;
		return this.http.post<Error[]>(url, project, this.httpOptions);
	}

	checkWellFormed(project: Project): Observable<Error[]> {
		const url = `http://localhost:7086/project/checkWellFormed`;
		return this.http.post<Error[]>(url, project, this.httpOptions);
	}

	getSubProblemDiagram(project: Project): Observable<Project> {
		//console.log(project)
		const url = `http://localhost:7086/project/getSubProblemDiagram`;
	...
	getBehIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
		const url = `http://localhost:7086/project/getBehIntList/${sgName}`;
	...
	getExpIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
		const url = `http://localhost:7086/project/getExpIntList/${sgName}`;
		return this.http.post<Phenomenon[]>(url, project, this.httpOptions);
	}

	// getPreIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
	// 	const url = `http://localhost:7086/project/getPreIntList/${sgName}`;
	// 	return this.http.post<Phenomenon[]>(url, project, this.httpOptions);
	// }

	getIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
		const url = `http://localhost:7086/project/getIntList/${sgName}`;
		return this.http.post<Phenomenon[]>(url, project, this.httpOptions);
	}

	getFullExpIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
		const url = `http://localhost:7086/project/getFullExpIntList/${sgName}`;
		return this.http.post<Phenomenon[]>(url, project, this.httpOptions);
	}
	...
	getScenarioGraph(project: Project, projectName: string): Observable<Project> {
		const url = `http://localhost:7086/project/getScenarioGraph?projectName=${projectName}`;
		return this.http.post<Project>(url, project, this.httpOptions);
	}

	getFullScenarioGraph(project: Project): Observable<Project> {
		const url = `http://localhost:7086/project/getFullScenarioGraph`;
		//console.log(url, project);
		return this.http.post<Project>(url, project, this.httpOptions);
	}

	getBreakdownScenarioGraph(project: Project): Observable<Project> {
		const url = `http://localhost:7086/project/getBreakdownScenarioGraph`;
		//console.log(url)
		return this.http.post<Project>(url, project, this.httpOptions);
	}

	getConnIntList(project: Project, sgName: string): Observable<Phenomenon[]> {
		const url = `http://localhost:7086/project/getConnIntList/${sgName}`;
		return this.http.post<Phenomenon[]>(url, project, this.httpOptions);
	}
  ...
  saveSpecification(project: Project) {
    const url = 'http://localhost:7086/project/saveSpecification';
    return this.http.post<StrategyAdvice[]>(url, project, this.httpOptions);
  }

  // 向后端申请保存当前项目作为修改后的lastProject
  recordLastProject(projectRecord: ProjectRecord) {
    console.log('recordLastProject中的projectRecord:', projectRecord);
    const url = 'http://localhost:7086/project/recordLastProject';
    return this.http.post<boolean>(url, projectRecord, this.httpOptions);
  }

  // 向后端申请构建初始追溯关系
  initTrace(project: Project) {
    const url = 'http://localhost:7086/project/initTrace';
    return this.http.post<Trace[]>(url, project, this.httpOptions);
  }

  // 向后端申请获取追溯关系，map['ID']存储当前项目版本的唯一标识（由用户名+项目名组成）,map['Project']为应用解耦策略修改后的项目内容
  checkTrace(checkTraceBody: CheckTraceBody) {
    const url = 'http://localhost:7086/project/checkTrace';
    return this.http.post<Trace[]>(url, checkTraceBody, this.httpOptions);
  }

  // 向后端申请获取数据依赖关系
  analyzeDataDependencies(project: Project) {
    const url = 'http://localhost:7086/project/analyzeDataDependencies';
    return this.http.post<Dependence[]>(url, project, this.httpOptions);
  }

  // 向后端申请获取控制依赖关系
  analyzeControlDependencies(project: Project) {
    const url = 'http://localhost:7086/project/analyzeControlDependencies';
    return this.http.post<Dependence[]>(url, project, this.httpOptions);
  }
