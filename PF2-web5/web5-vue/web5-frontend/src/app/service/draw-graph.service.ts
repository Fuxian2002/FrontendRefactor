...
export class DrawGraphService {
  ...
  //==============================WebSocket=========================
  openWebSocket() {
    this.ws = new WebSocket('ws://localhost:7086/webSocket');
    var that = this
  ...
  register(title) {
    ...
    if (this.project.title != undefined) {
      this.unregister(this.projectAddress);
    }
    var message = {
      "type": "register",
      "title": title,
  ...
  initProject(title) {
    ...
    tmpProject.init(title, this.project);
  ...
  enterTable(index, project) {
    ...
    switch (index) {
      ...
      case 2:
        ...
        break;
      case 3:
        // 不需要drawFullSenarioGraph，故将这部分代码注释掉
  ...
  getProject(projectAddress: string, version: string): void {
      var that = this;
  ...
  setProject(project) {
    ...
    // this.projectService.initTrace(project).subscribe(
    //   traceList => {
    //     this.communicationService.traces = traceList;
    //     this.communicationService.tracesToStr();
    //   }
    // );
  }
  ...
  dealConstraint(project) {
    ...
    for (let temp of project.problemDiagram.constraintList) {
      for (let phe of temp.phenomenonList) {
        if (phe.phenomenon_constraint == null) {
          //console.log('null-------')
          phe.phenomenon_constraint = true
        }
      }
    }
  }
  ...
  drawCtrlNodeList(ctrlNodeList: CtrlNode[], elementList, graph, id: string) {
    ...
    for (var i = 0; i < ctrlNodeList.length; i++) {
      ...
      if(ctrlNode != null && ctrlNode.node_type != null) {
  ...
  drawLineList(lineList: Line[], elementList, graph, paper) {
    ...
    for (var i = 0; i < lineList.length; i++) {
      ...
      for (var j = 0; j < elementList.length; j++) {
        if (from != null && from.node_type + from.node_no === elementList[j].attr('root').title) {
          fromElement = elementList[j];
        } else if (to != null && to.node_type + to.node_no === elementList[j].attr('root').title) {
  ...
  drawLine(line: Line, fromElement, toElement, graph, paper) {
    ...
    var vertices = [];
    if(line.turnings != undefined && line.turnings != null) {
      var splitAdd = line.turnings.split(',');
      var dict = [];
      for(var i=0;i<splitAdd.length;i++) {
        if(i % 2 == 0) {
          dict = [];
          dict['x'] = parseInt(splitAdd[i]);
        }
        else {
          dict['y'] = parseInt(splitAdd[i]);
          vertices.push(dict);
        }
      }
    }
    //console.log('vertices:', vertices);
    //console.log('line.line_type + line.line_no:', line.line_type + line.line_no);
    switch (line.line_type) {
      case 'BehOrder':
        ...
        //console.log('drawLine:source:', fromElement);
    ...
    // console.log(link)
    if (link != undefined) {
      if (line != null && line.fromNode != null && line.fromNode.node_type == "Decision") {
        link.labels([{
          attrs: {
            text: {
              text: line.condition
      ...
      paper.on('link:mouseover', function (linkView) {
        //console.log('捕获到mouseover动作:');
        //console.log('linkView.model:', linkView.model);
        //console.log('linkView.model.attributes.attrs.root.title:', linkView.model.attributes.attrs.root.title);
        //console.log('linkView.model.attributes.vertices:', linkView.model.attributes.vertices);
        if(linkView.model.attributes.vertices != undefined && line.line_type + line.line_no == linkView.model.attributes.attrs.root.title) {
          var vertices = linkView.model.attributes.vertices.flat(Infinity);
          var turn = [];
          for(var i=0;i<vertices.length;i++) {
            turn.push(vertices[i]['x']);
            turn.push(vertices[i]['y']);
          }
          //console.log('turnings:', turn);
          line.turnings = turn.toString();
          //console.log('line.turnings:', line.line_type + line.line_no + ' ' + line.turnings);
        }
      });
  ...
  changeElementPosition(position) {
    ...
    if (type == "machine") {
      ...
      var tmpProject = new Project();
      tmpProject.initProject(this.project);
      tmpProject.changeMachinePositionNew(name, position);
      this.project = tmpProject;
      ...
      var tmpProject = new Project();
      tmpProject.initProject(this.project);
      tmpProject.changeRequirementPosition(name, position);
      this.project = tmpProject;
      return;
    }
  }
  ...
  deleteMachinews(old: Machine) {
    var tmpProject = new Project();
    tmpProject.initProject(this.project);
    tmpProject.deleteRelatedLink(this.project.contextDiagram.machine.machine_shortName);
    this.project = tmpProject;
  ...
  deleteProblemDomainws(pd1: ProblemDomain) {
    ...
    for (let graph of this.graphs) {
      var tmpProject = new Project();
      tmpProject.initProject(this.project);
      tmpProject.deleteRelatedLink(shortName);
      this.project = tmpProject;
      //this.project.deleteRelatedLink(shortName)
      for (let element of graph.getCells()) {
  ...
  changeRequirementDetail(graph) {
    ...
    // this.projectService.sendProject(this.project)
    let newR = Requirement.newRequirementWithOld(old, description)
  ...
  drawInterfaces(interfaceList: Interface[], elementList, graph: joint.dia.Graph) {
    ...
    for (var i = 0; i < interfaceList.length; i++) {
      ...
      console.log(this.project)
      if (this.getExternalEntityByShortName(from)!=null
          || this.getExternalEntityByShortName(to)!=null){
            console.log("------------------------return",int.interface_name)
          continue;
          }
      for (var j = 0; j < elementList.length; j++) {
  ...
  getExternalEntityByShortName(shortName) {
    for (let pro of this.project.intentDiagram.externalEntityList) {
      console.log(shortName, pro)
      if (pro.externalentity_shortname == shortName)
        return pro;
    }
    // console.log(shortName, this.project.intentDiagram.externalEntityList)
    return null;
  }
  ...
  changeInterfaceDetail() {
    ...
    var tmpProject = new Project();
    tmpProject.initProject(this.project);
    let int = Interface.newInterfaceWithOld(this.interface, this.phenomenonList, tmpProject.getDescription(this.interface.interface_name, this.phenomenonList));
    this.project = tmpProject;
  ...
  changeReferenceDetail() {
    ...
    var tmpProject = new Project();
    tmpProject.initProject(this.project);
    let ref = Reference.newReferenceWithOld(this.reference, this.phenomenonList, tmpProject.getDescription(this.reference.reference_name, this.phenomenonList));
    this.project = tmpProject;
  ...
  changePhenomenon1(phenomenon, phenomenonList) {
    ...
    let selectedIndex = (selectElement as any).selectedIndex;
    let phenomenonType = (selectElement as any).options[selectedIndex].text;
    console.log('changePhenomenon1.phenomenonType:', phenomenonType);
    //from
    ...
    for (let phe of relatedPhes) {
      if (phe.phenomenon_name == phenomenonName) {
        ...
        flag = true
        console.log('存在相关连接:', phe.phenomenon_no, phe.phenomenon_name, phe.phenomenon_from, phe.phenomenon_to, phe.phenomenon_type,  phe.phenomenon_constraint);
      }
    }
     */
