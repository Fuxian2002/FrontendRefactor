...
public class FileController {
	...
	public boolean upload(@RequestParam("xmlFile") MultipartFile uploadFile,
...
//		}
		fileService.addFile(uploadFile, projectName);
	...
	public boolean uploadOwl(@RequestParam String username, @RequestParam("xmlFile") MultipartFile uploadFile,
		...
		if (type.contentEquals("eowl")) {
			...
			fileService.addFile(uploadFile, eOwlRootAddress);
			fileService.copyEOwl(userAdd, projectAddress, owlBranch);// 存入PF/Project文件夹下
		} else {
			fileService.setPOwl(owlBranch);
			fileService.addFile(uploadFile, pOwlRootAddress);
			fileService.copyPOwl(userAdd, projectAddress, owlBranch);// 存入PF/Project文件夹下
		}
		return true;
	}
	...
	public boolean setProject(@RequestParam String username, @PathVariable("projectAddress") String projectAddress) {
		...
		String userAdd = getUserAdd(username);
		boolean result = fileService.setProject(userAdd+branch+"/"+ reqModelDir +"/", branch);
		return result;
	}
	...
	public boolean moveOwl(@RequestParam String username, @PathVariable("projectAddress") String projectAddress,
		...
		String userAdd = getUserAdd(username);
		if (type.contentEquals("eowl"))
			return fileService.copyEOwl(userAdd, projectAddress, owl);
		else if (type.contentEquals("powl"))
			return fileService.copyPOwl(userAdd, projectAddress, owl);
		else
			return false;
	}
	...
	@ResponseBody
	public Project getProject(@RequestParam String username, @PathVariable("projectName") String projectName, @PathVariable("version") String version) {
		System.out.printf("getProject:" + projectName + "\n");
	...
	@RequestMapping(value = "/saveProject/{projectAddress}/{userName}", method = RequestMethod.POST)
	@ResponseBody
	public boolean saveProject(@PathVariable("projectAddress") String projectAddress, @PathVariable("userName") String userName,
							   @RequestBody Project project) {
		return fileService.saveProject(getUserAdd(userName)+projectAddress+"/", project, projectAddress);
		//return fileService.saveProject(project, getUserAdd(userName) + projectAddress + "/");
	}

	@RequestMapping(value = "/createVersion/{projectAddress}/{userName}", method = RequestMethod.POST)
	@ResponseBody
	public boolean createVersion(@PathVariable("projectAddress") String projectAddress,
								 @PathVariable("userName") String userName) throws IOException {
		// 以当前时刻（精确到分钟）为名创建文件夹，并将当前项目文件（上下文图、问题图、情景图）放到该文件夹下的需求模型子目录中
		return fileService.createVersion(getUserAdd(userName)+projectAddress+"/");
	}
	...
	public boolean format(@RequestParam String username, @PathVariable("projectAddress") String projectAddress,
		...
		result = fileService.format(getUserAdd(username)+branch+"/"+ reqModelDir +"/", project, branch);
		return result;
	}
	...
	public void download(@PathVariable("projectAddress") String projectAddress, @PathVariable("username") String username, HttpServletResponse resp) {
		...
		fileService.download(getUserAdd(username)+branch+"/"+ reqModelDir +"/", fileName, resp, branch);
	}
	...
	public void format_download(@PathVariable("projectAddress") String projectAddress, @PathVariable("username") String username, HttpServletResponse resp) {
		...
		String branch = projectAddress;
		String fileName = projectAddress.split("/")[0];
		fileService.format_download(getUserAdd(username)+branch+"/"+ reqModelDir +"/", fileName, resp, branch);
	}
	...
	public ArrayList<MyOntClass> getProblemDomains(@RequestParam String username, @PathVariable("owlAdd") String owlAdd,
		...
		re = fileService.GetProblemDomains(getUserAdd(username), owlName, branch);
		return re;
	}
	...
	public String[] getNodes(@RequestParam String username, @PathVariable("projectAddress") String projectAddress,
		...
		String branch = projectAddress;
		if (type.equals("powl"))
			re = fileService.pOntShowGetNodes(getUserAdd(username), fileName, nodeName, branch);
		else if (type.equals("eowl"))
			re = fileService.eOntShowGetNodes(getUserAdd(username), fileName, nodeName, branch);
		return re;
	}
