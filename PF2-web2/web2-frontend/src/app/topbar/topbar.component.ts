...
export class TopbarComponent implements OnInit {
  ...
  test() {
    window.location.href = 'http://localhost:4203/help';
    //this.fileService.getProject('LightController723');
    // console.log('测试：');
    // const url = 'http://localhost:7085/file/getProject/LightController723';
    // //this.http.get('http://localhost:7085/file/getProject/LightController723');
  ...
  download() {
    ...
    // var url = `http://localhost:7085/file/download/${project.title}?username=${username}`;
    var url = `http://localhost:7085/file/download/${project.title}/${username}`;
    //console.log(url);
  ...
  format_download() {
    ...
    var project = this.dg_service.project;
    var username = this.fileService.getUserName()
    var url = `http://localhost:7085/file/formatdownload/${project.title}/${username}`;
    var form = document.createElement('form');
    document.body.appendChild(form);
    form.style.display = "none";
    form.action = url;
