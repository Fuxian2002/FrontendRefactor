...
export class RightbarComponent implements OnInit {
  ...
  back() {
    if(this.step == 1){
      var next = confirm('进入需求描述界面？');
        if (next) {
          var next2 = confirm('是否保存对当前项目的修改？');
          if(next2){
            this.fileService.saveProject(this.project.title, this.project).subscribe();
            // window.location.href = 'http://localhost:4203/?projectname=' + this.project.title;
            window.location.href = 'http://localhost/PF2-web1/?projectname=' + this.project.title;
          }else{
            // window.location.href = 'http://localhost:4203/?projectname=' + this.project.title
            window.location.href = 'http://localhost/PF2-web1/?projectname=' + this.project.title
            + '&version=' +this.dg_service.reqVersion;
          }
  ...
  check() {
    ...
    switch (this.step) {
        ...
        this.projectService.checkCorrectProblem(project_to).subscribe(
          errors => {
            ...
            if (this.projectService.getRes(errorList)) {
              ...
              alert('There are errors in Problem Diagram!');
              // flag = false;
              this.step = 1;
            }
            this.projectService.stepChange(this.step);
      ...
      case 2:
        ...
        // var flag = true;
        // var project_to = this.dg_service.project;
        this.reqVersion = this.dg_service.reqVersion;
        // this.projectService.checkCorrectProblem(project_to).subscribe(
        //   errors => {
        //     errorList = errors;
        //     this.sendErrors(errorList);
        ...
        //   }
        // );
        this.projectService.getScenarioGraph(this.project, this.project.title, this.reqVersion).subscribe(
          project => {
            if(project == undefined){
              ...
              console.log('case 2 this.project:', this.project);
      ...
      //               this.fileService.saveProject(this.project.title, this.project).subscribe();
      //               location.href = 'http://localhost/syn/?projectname=' + this.project.title;
      ...
      // case 8:
      //   this.projectService.saveSpecification(this.project,userName).subscribe(res => {
      //     if(res) {
