...
export function useDrawGraph() {
  ...
  const initPapers = () => {
    // Remove existing papers from DOM before clearing to prevent stacking orphaned SVGs
    papers.value.forEach(p => {
      if (p) {
        try { (p as any).remove(); } catch (e) {}
      }
    });
    graphs.value = [];
    papers.value = [];
    ...
    // Initialize Context and Problem Diagrams (Tabs 1 & 2)
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (!width && el[0].parentElement) {
          width = el[0].parentElement.clientWidth;
    ...
    // Scenario graph papers are created lazily (when the tab is first shown via ensureScenarioGraphPaper).
    // Initializing them here while hidden causes JointJS ref-based attrs to compute with 0-sized text.
  ...
  const attachPaperEvents = (paper: joint.dia.Paper, graph: joint.dia.Graph, index: number) => {
    ...
    // Cell click - Unified selection (element or link)
    paper.on('cell:pointerclick', (cellView: any) => {
        ...
        if (clickedToolView.value) {
            try { clickedToolView.value.hideTools(); } catch {}
            clickedToolView.value = null;
        }
        const model = cellView.model;
        if (model.isLink && model.isLink()) {
            clickedToolView.value = cellView;
            try { cellView.showTools(); } catch {}
        } else {
            clickedSceElement.value = model;
            clickedSceElement.value.attr('body/stroke', '#1890ff');
            clickedSceElement.value.attr('body/strokeWidth', 3);
        }
    });
  ...
  const drawIntNode = (id: string, intNode: any, graph: joint.dia.Graph) => {
    ...
    let fillColor = 'rgb(135,206,250)';
    if (intNode.node_type === 'BehInt') {
        fillColor = 'rgb(189,215,238)'; // Blue
    } else if (intNode.node_type === 'ConnInt') {
        fillColor = 'rgb(197,224,180)'; // Green
    } else if (intNode.node_type === 'ExpInt') {
        fillColor = 'rgb(255,230,153)'; // Yellow
    }

    let labelText = 'int' + intNode.node_no;
    if (intNode.pre_condition) {
       labelText = '* int' + intNode.node_no;
    } else if (intNode.post_condition) {
       labelText = 'int' + intNode.node_no + '*';
    }
    ...
    intElement.attr({
      body: {
        ...
        fill: fillColor,
        strokeWidth: 1,
        strokeDasharray: intNode.node_type === 'ExpInt' ? '8,2' : undefined
      },
      label: {
        text: labelText,
        fontSize: 25,
    ...
    // Also save size on resize
    intElement.on('change:size', function(element1: any, size: any) {
        intNode.node_w = size.width;
        intNode.node_h = size.height;
        saveInt(id.substring(0, id.length - 1), intNode);
    });
  ...
  const drawSubProblemDiagram = (subProblemDiagram: SubProblemDiagram) => {
  ...
  const drawNewSpd = (proj: Project) => {
    ...
    for (let i = 0; i < subProblemDiagramList.length; i++) {
      drawSubProblemDiagram(subProblemDiagramList[i]);
    }
  };
  ...
  const drawNewSenarioGraph = (newScenarioGraph: ScenarioGraph) => {
    ...
    // If element is hidden (v-show=false), clientWidth is 0. Use parent width or fallback.
    ...
    // After all elements are drawn, fit the paper to actual content dimensions
    setTimeout(() => {
      try {
        paper.fitToContent({ padding: 50, minWidth: width, minHeight: height, allowNewOrigin: 'any' });
      } catch (e) {
        // fitToContent may not exist in all JointJS 3.x builds; ignore
      }
    }, 100);
  };

  // Force all element views in a paper to re-render their attrs.
  // Needed after a hidden container (v-show=false) becomes visible, because JointJS
  // cannot measure SVG text while the element is hidden (getBBox returns 0).
  ...
  // Ensure the paper for a scenario graph tab exists, creating it lazily if needed.
  const ensureScenarioGraphPaper = (title: string) => {
    const id = title + 'M';
    const existing = papers.value.find(p => p && (p as any).id === id);
    if (existing) {
      // Paper already exists; force re-render now that container is visible
      forceUpdatePaper(existing);
      return;
    }
    const sg = project.value?.scenarioGraphList?.find(s => s.title === title);
    if (sg) drawNewSenarioGraph(sg);
  };

  // Ensure context diagram paper exists and is up to date.
  const ensureContextDiagramPaper = () => {
    if (papers.value[0]) {
      forceUpdatePaper(papers.value[0]);
    } else if (project.value?.contextDiagram) {
      drawContextDiagram1(project.value.contextDiagram);
    }
  };

  // Ensure problem diagram paper exists and is up to date.
  const ensureProblemDiagramPaper = () => {
    if (papers.value[1]) {
      forceUpdatePaper(papers.value[1]);
    } else if (project.value?.problemDiagram) {
      drawProblemDiagram1(project.value.problemDiagram);
    }
  };

  const refreshScenarioGraph = (title: string) => {
    const sg = project.value.scenarioGraphList?.find(s => s.title === title);
    if (!sg) return;
    ...
    const paperIndex = papers.value.findIndex(p => p && (p as any).id === id);
    if (paperIndex !== -1) {
        if (papers.value[paperIndex]) {
            try { papers.value[paperIndex].remove(); } catch (e) {}
        }
        papers.value.splice(paperIndex, 1);
        graphs.value.splice(paperIndex, 1);
    }
    
    drawNewSenarioGraph(sg);
  };

  const refreshContextDiagram = () => {
    // Destroy existing paper
    if (papers.value[0]) {
        try { papers.value[0].remove(); } catch (e) {}
        papers.value[0] = undefined as any;
    }
    if (graphs.value[0]) {
        graphs.value[0] = undefined as any;
    }
  ...
  const refreshProblemDiagram = () => {
    // Destroy existing paper
    if (papers.value[1]) {
        try { papers.value[1].remove(); } catch (e) {}
        papers.value[1] = undefined as any;
    }
    if (graphs.value[1]) {
        graphs.value[1] = undefined as any;
    }
  ...
  const openWebSocket = () => {
    ws.value = new WebSocket('ws://localhost:7085/webSocket');
    ws.value.onopen = () => {
