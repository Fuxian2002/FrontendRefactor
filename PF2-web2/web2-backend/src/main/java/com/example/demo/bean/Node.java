...
public class Node {
	...
	private int node_y;
	private List<Node> node_fromList;  // 节点通过序关系连接的前置节点（这并不是情景图的必要信息，应该是为了方便遍历额外添加的）
	private List<Node> node_toList;  // 节点通过序关系连接的后置节点
	private Phenomenon pre_condition;  // （貌似没有使用，一直为空）
	private Phenomenon post_condition;  // （貌似没有使用，一直为空）
