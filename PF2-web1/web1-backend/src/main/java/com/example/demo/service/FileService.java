...
public class FileService {
	...
	public void addEOwl(MultipartFile file, String branch) {
		addFile(file, addressService.geteOwlRootAddress(), branch);
	}

	public void addPOwl(MultipartFile file, String branch) {
		addFile(file, addressService.getpOwlRootAddress(), branch);
	}
...
//			e.printStackTrace();
//		}
//		System.out.println("addFile第一阶段完成");
//		// 保存文件
//		saveUploadFile(file, userAdd);
//		System.out.println("addFile第二阶段完成, userAdd:" + userAdd);
//
//		try {
...
//		System.out.println("addFile第三阶段完成");
//	}
	public void addFile(MultipartFile file, String userAdd, String branch) {
		// 保存文件
		saveUploadFile(file, userAdd);
	...
	public boolean setOwl(String branch) {
		return set(addressService.getOwlRootAddress(), branch);
	}

	public boolean setPOwl(String branch) {
		return set(addressService.getpOwlRootAddress(), branch);
	}

	public boolean setEOwl(String branch) {
		return set(addressService.geteOwlRootAddress(), branch);
	}
...
//	}
	public boolean set(String userAdd, String branch) {
		...
		if(!dir.exists() || !dir.isDirectory()) {
			...
			dir.mkdirs();
			if(!userAdd.endsWith(addressService.getReqModelDir()+"/")) {
	...
	public void setRootProject(String userAdd, String branch) {
...
//			fileListElement.addElement("RequirementVersion");
	...
	public void addpfFile(MultipartFile file) {
		saveUploadFile(file, addressService.getPfRootAddress());
	}
	public Project getCorrectProject(String userAdd, String version,
									 String branch) {
		System.out.println("~~~getCorrectProject~~~~~~userAdd: "+userAdd+" branch:"+branch);
		Project project = getProject(userAdd, branch, version);
		if (project == null) {
			project = new Project();
			project.setTitle(branch);
			project.setIntentDiagram(new IntentDiagram());
			project.setContextDiagram(new ContextDiagram());
			project.setProblemDiagram(new ProblemDiagram());
			return project;
		} else
			// 将#修改为_
			return Project.getCorrectProject(project);
	}
	public String getNotNullPf(String userAdd, String version, String branch) {
		System.out.println("---getNotNullPf:"+userAdd+" version: "+version+" branch: "+branch);
		Project project = getCorrectProject(userAdd, version, branch);
		return ProblemEditor.project2pf(project);
	}
	// 保存pf文件到userAdd
...
//	}
	public boolean saveProject(String userAdd, Project project, String branch) {
		System.out.println("saveProject:"+userAdd);
		setProject(userAdd, branch);// 创建并切换分支
		if(!userAdd.endsWith(addressService.getReqModelDir() +"/")){
			userAdd += addressService.getReqModelDir() +"/";
		}
	...
	public boolean saveProject(String userAdd, String branch, Project project) {
		...
		System.out.println("tmp_ID1"+tmp_ID);
...
//		}
	...
	/**
	 * @Description: 保存project到userAdd目录下，设置分支但不commit
	 ...
	 * @throws
	 */
	public boolean saveProjectWithoutCommit(String userAdd, Project project,
											String branch) {
		setProject(userAdd, branch);// 创建并切换分支
		userAdd += addressService.getReqModelDir() +"/";
	 ...
	 * @throws
	 */
	public boolean savePf(String userAdd, String projectAddress, String pf) {
		setProject(userAdd, projectAddress);
		// 保存pf文件
		String filename = userAdd + projectAddress + ".pf";
...
//		try {
//			GitUtil.RecordUploadProjAt("savepf", userAdd, ".");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
		return true;
...
//			// XMLWriter out = new XMLWriter(new FileWriter(file), format);
	...
	private ContextDiagram saveContextDiagram(String userAdd, String projectAddress, ContextDiagram contextDiagram) {
		...
		if (machine != null && machine.getShortname() != null) {
			System.out.println("machine:"+machine.getMachine_shortName()+ " "+ machine.getShortname());
		...
		if (problemDomainList!=null && problemDomainList.size() > 0) {
		...
		if (constraintList!=null && constraintList.size() > 0) {
			for (int i = 0; i < constraintList.size(); i++) {
				...
				for (int j = 0; j < phenomenonElementList.size(); j++) {
					...
					String phenomenon_shortname = tmp_p.getPhenomenon_shortname();
					String phenomenon_type = tmp_p.getPhenomenon_type();
					if(tmp_p.getPhenomenon_from()==null){
						tmp_p.setPhenomenon_from("");
					}
					if(tmp_p.getPhenomenon_to()==null){
						tmp_p.setPhenomenon_to("");
					}
					String phenomenon_from = tmp_p.getPhenomenon_from().replaceAll("\n", "&#x000A");
					String phenomenon_to = tmp_p.getPhenomenon_to().replaceAll("\n", "&#x000A");
					String phenomenon_constraint = tmp_p.getPhenomenon_constraint();
		...
		for(Node intNode: intNodeList) {
			...
			if(intNode.getNode_type().equals("BehInt")) {
				...
				if(pre_condition != null) {
					...
					preConditionElement.addAttribute("phenomenon_shortname", pre_condition.getPhenomenon_shortname());
				...
				if(post_condition != null) {
					...
					postConditionElement.addAttribute("phenomenon_shortname", post_condition.getPhenomenon_shortname());
...
//		File xmlFile = new File(rootAddress + projectAddress + "/" + SG.getTitle() + ".xml");
	...
	private void saveSubProblemDiagram(String address, SubProblemDiagram spd) {
		...
		requireElement.addAttribute("requirement_shortname", requirement.getRequirement_shortname());
		...
		for(Interface inte: interfaceList) {
			...
			for(Phenomenon phenomenon: phenomenonList) {
				...
				phenomenonElement.addAttribute("phenomenon_shortname", phenomenon.getPhenomenon_shortname());
...
//		File xmlFile = new File(rootAddress + projectAddress + "/" + spd.getTitle() + ".xml");
	...
	public Project getProject(String userAdd, String branch) {
		...
		try {
			File xmlFile = new File(userAdd + addressService.getReqModelDir() +"/Project.xml");
			if(!xmlFile.exists()){
				...
				if(projectDir.exists()){
					...
					saveProject(userAdd + addressService.getReqModelDir() +"/", project, branch);
					return project;
				}else {
					return null;
				}
			...
			Element senarioGraphListElement = fileListElement.element("ScenarioGraphList");
...
//			Element newSenarioGraphListElement = fileListElement.element("NewSenarioGraphList");
			...
			System.out.println("senarioGraphListElement:"+senarioGraphListElement);
			...
			IntentDiagram intentDiagram = getIntentDiagram(userAdd+ addressService.getReqModelDir() +"/", branch, intentDiagramName);
			ContextDiagram contextDiagram = getContextDiagram(userAdd+ addressService.getReqModelDir() +"/", branch, contextDiagramName);
			ProblemDiagram problemDiagram = getProblemDiagram(userAdd+ addressService.getReqModelDir() +"/", branch, problemDiagramName);
			...
			if(traceListElement != null) {
				traceList = getTraceList(userAdd+ addressService.getReqModelDir() +"/", branch, traceListElement.getText());
			}
			if(dataDependenceListElement != null) {
				dataDependenceList = getDataDependenceList(userAdd+ addressService.getReqModelDir() +"/", branch, dataDependenceListElement.getText());
			}
			if(controlDependenceListElement != null) {
				controlDependenceList = getControlDependenceList(userAdd+ addressService.getReqModelDir() +"/", branch, controlDependenceListElement.getText());
			}
			...
			if(senarioGraphListElement != null) {
				...
				List<ScenarioGraph> scenarioGraphList = getScenarioGraphList(userAdd+ addressService.getReqModelDir() +"/", senarioGraphElementList, "haveRequirement");
...
//				List<?> newSenarioGraphElementList = newSenarioGraphListElement.elements("SecnarioGraph");
...
//			}
			if(subProblemDiagramListElement != null) {
				...
				List<SubProblemDiagram> subProblemDiagramList = getSubProblemDiagramList(userAdd+ addressService.getReqModelDir() +"/", subProblemDiagramElementList);
...
//			String contextDiagramName = contextDiagramElement.getText();
//			String problemDiagramName = problemDiagramElement.getText();
//			IntentDiagram intentDiagram = getIntentDiagram(userAdd, branch, intentDiagramName);
...
//			List<Dependence> controlDependenceList = new ArrayList<>();
//			if(traceListElement != null) {
//				traceList = getTraceList(userAdd, branch, traceListElement.getText());
//			}
//			if(dataDependenceListElement != null) {
//				dataDependenceList = getDataDependenceList(userAdd, branch, dataDependenceListElement.getText());
//			}
//			if(controlDependenceListElement != null) {
//				controlDependenceList = getControlDependenceList(userAdd, branch, controlDependenceListElement.getText());
//			}
//
//			project.setTitle(title);
//			project.setIntentDiagram(intentDiagram);
...
////			if(subProblemDiagramListElement != null) {
////				List<?> subProblemDiagramElementList = subProblemDiagramListElement.elements("SubProblemDiagram");
////				List<SubProblemDiagram> subProblemDiagramList = getSubProblemDiagramList(userAdd, subProblemDiagramElementList);
////				project.setSubProblemDiagramList(subProblemDiagramList);
////			} else {
////				List<SubProblemDiagram> spdList = new ArrayList<SubProblemDiagram>();
////				project.setSubProblemDiagramList(spdList);
...
//			}
			...
			List<ProblemDomain> problemDomainList = getProblemDomainList(problemDomainListElement,null);
...
//			problemDiagram.setContextDiagram(contextDiagram);
	...
	private List<ScenarioGraph> getScenarioGraphList(String address, List<?> senarioGraphElementList, String requirementFlag) {
		...
		for(Object sge : senarioGraphElementList) {
			...
			System.out.println("senarioGraphName: "+senarioGraphName);
			SAXReader saxReader = new SAXReader();
			try {
//				File senarioGraphFile = new File(rootAddress + projectAddress + "\\"+ version + "\\" + senarioGraphName + ".xml");
	...
	private Phenomenon getPhenomenon(Element phenomenonElement) {
		...
		String phenomenon_shortname = phenomenonElement.attributeValue("phenomenon_shortname");
		...
		phenomenon.setPhenomenon_shortname(phenomenon_shortname);
...
//				controlNode.setNode_consition2(node_consition2);
	...
	private List<Phenomenon> getPhenomenonList(String address){
		...
		if(!address.contains(addressService.getReqModelDir()+"/")){
			address=address+ addressService.getReqModelDir()+"/";
		}
		try {
...
//			requirement_shortname = requirement_context;
			requirement_shortname = "requirement"+ requirement_no;
		}
//		System.out.println("requirement_shortname:"+requirement_shortname);
	...
	private List<Phenomenon> getPhenomenonList(List<?> phenomenonElementList) {
		...
		for (Object object : phenomenonElementList) {
			...
			String phenomenon_name = phenomenonElement.attributeValue("phenomenon_name");
			String phenomenon_shortname = phenomenonElement.attributeValue("phenomenon_shortname");
			if(phenomenon_shortname==null || phenomenon_shortname.equals("")){
				phenomenon_shortname="phe"+phenomenon_no;
			}
			String phenomenon_type = phenomenonElement.attributeValue("phenomenon_type");
...
//			e.printStackTrace();
//		}
		boolean result = fileToZip(userAdd, addressService.getDownloadRootAddress(), fileName);
...
//		}
		...
		try {
			...
			// 输入流：本地文件路径
			String downloadPath = addressService.getDownloadRootAddress();
...
//		}
		...
		try {
			...
			resp.setHeader("Cache-Control", "no-cache");
			// 输入流：本地文件路径
			in = new DataInputStream(new FileInputStream(new File(addressService.getDownloadRootAddress() + fileName)));
			// 输出流
			out = resp.getOutputStream();
			// 输出文件
			int bytes = 0;
	...
	public boolean fileToZip(String sourceFilePath, String zipFilePath, String fileName) {
		...
		try {
			...
			// Ensure parent directory exists for zip file
			...
			compressRecursive(sourceFile, sourceFile, zos);
	...
	private void compressRecursive(File rootDir, File sourceFile, ZipOutputStream zos) throws IOException {
		if (sourceFile.isDirectory()) {
			File[] files = sourceFile.listFiles();
			if (files != null) {
				for (File file : files) {
					compressRecursive(rootDir, file, zos);
				}
			}
		} else {
			String relativePath = sourceFile.getAbsolutePath().substring(rootDir.getAbsolutePath().length());
...
//		return searchbranch(userAdd);
	...
	public List<String> searchPOwl() {
		return searchbranch(addressService.getpOwlRootAddress());
	}

	public List<String> searchEOwl() {
		return searchbranch(addressService.geteOwlRootAddress());
	}
...
//	}
	...
	public List<String> searchOwlVersion(String branch) {
		return searchVersion1(addressService.getOwlRootAddress(), branch);
...
//		}
	...
	public boolean copyEOwl(String userAdd, String proBranch, String owlBranch) {
		return copyOwl(userAdd, proBranch, addressService.geteOwlRootAddress(), owlBranch);
	}

	public boolean copyPOwl(String userAdd, String proBranch, String owlBranch) {
		return copyOwl(userAdd, proBranch, addressService.getpOwlRootAddress(), owlBranch);
	}
...
//		}
		// 移动pf文件
		String fromPath = addressService.getPfRootAddress() + fileName + ".pf";
...
//		}
	...
	public void downloadFile(String userName, String projectName, String version, String fileName, HttpServletResponse response) {
		...
		String address;
		if(userName.equals("")) {
			address = addressService.getRootAddress();
		}else {
			address = addressService.getUserAddress() + userName + "/";
		}
	...
	public List<String> searchEowlVersion(String branch) {
		List<String> projectVersions = new ArrayList<String>();
		List<versionInfo> versions = searchVersionInfo(addressService.geteOwlRootAddress(), branch);
		if(versions != null) {
			for(versionInfo version: versions) {
				projectVersions.add(version.getTime());
			}
	...
	public boolean copyOwl(String userName, String projectName, String project_version, String owl, String eowl_version) {
		...
		boolean res = false;
		// 切换owl分支读取owl文件
		List<versionInfo> ewol_versions = searchVersionInfo(addressService.geteOwlRootAddress(), owl);
		String address;
		if(userName.equals("")) {
			address = addressService.getRootAddress();
		}else {
		...
		try {
			//切换到eowl的当前版本
			if(!GitUtil.currentBranch(addressService.geteOwlRootAddress()).equals(owl)) {
				GitUtil.gitCheckout(owl, addressService.geteOwlRootAddress());
			}
			GitUtil.rollback(addressService.geteOwlRootAddress(), eowl_version, ewol_versions);
			//切换到project的当前版本
			if(!GitUtil.currentBranch(address).equals(projectName)) {
				GitUtil.gitCheckout(projectName, address);
			}
		...
		// 移动owl文件
		String fromPath = addressService.geteOwlRootAddress() + owl + ".owl";
	...
	public EnvEntity getEnvEntity(String userName, String branch) {
		String address;
		if(userName.equals("")) {
			address = addressService.getRootAddress();
		}else {
		...
		try {
			if(!GitUtil.currentBranch(address).equals(branch)) {
				GitUtil.gitCheckout(branch, address);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public EnvEntity getEnvEntity(String address) {
