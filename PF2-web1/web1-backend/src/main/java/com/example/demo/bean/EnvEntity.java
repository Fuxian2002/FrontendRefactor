...
//			System.out.println(c.getLocalName());
			...
			if (!(type = isSuperC(c)).equals("No")) {
				System.out.println(c.getLocalName()+"type: "+type);
				MyOntClass o = new MyOntClass();
				o.setId(id++);
				o.setName(c.getLocalName());
				o.setType(type);
				if(!if_en){
					o.addValues(getRestrictionValue(c, "has_attribute"));
					o.addInstructions(getRestrictionValue(c,"has_instruction"));
					o.addSignals(getRestrictionValue(c,"has_signal"));
					ArrayList<String> timed_aut =getRestrictionValue(c,"has_timed_automaton");
					if(timed_aut.size()>0){
						o.setIsdynamic(true);
						if(timed_aut.size()==1){
							...
							// 获取状态机的状态
							o.addStates(getRestrictionValue(sm, "has_state"));
						} else
					...
					if (type.equals("DataProcessDevice") || type.equals("LaunchDevice")
						...
						ArrayList<String> timed_aut =getRestrictionValue(c,"has_SHA");
						if(timed_aut.size()>0){
							o.setIsdynamic(true);
							if(timed_aut.size()==1){
								o.setTA_name(timed_aut.get(0));
					...
					}else if(type.equals("Analyzer") || type.equals("Commander")
							|| type.equals("Maintainer")){
						ArrayList<String> timed_aut =getRestrictionValue(c,"has_command_SHA");
						if(timed_aut.size()>0){
							o.setIsdynamic(true);
							if(timed_aut.size()==1){
								o.setTA_name(timed_aut.get(0));
