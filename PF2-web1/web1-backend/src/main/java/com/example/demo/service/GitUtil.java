...
public class GitUtil {
	...
	// git init
	public static Repository createRepository1(String directoryName) {
		// prepare a new folder(directory)
		File localPath = new File(directoryName);
		if (!localPath.mkdirs()) {
			logger.info(localPath + " 目录创建失败...该目录已存在...");
		}
		// create the directory
	...
	public static boolean createBranch(String branchName, String directoryName, String startPoint)
		...
		try (Repository repository = getRepository(directoryName)) {
			try (Git git = new Git(repository)) {
				...
				System.out.println("branchName:" + branchName + "  directoryName:" + directoryName + "  startPoint:" + startPoint);
	 ...
	 * @param @param localFileOrDirectoryName
	 * @param @return
	 * @param @throws IOException
	 * @param @throws GitAPIException 设定文件
	 * @return boolean 返回类型
	 * @throws
	 */
	public static boolean commit(String commitMessage, String directoryName,
								 String name, String email) {
		try (Repository repository = getRepository(directoryName)) {
			try (Git git = new Git(repository)) {
				git.add().addFilepattern(".").call();
				git.commit().setMessage(commitMessage).setAuthor(name, email)
						.call();
				return true;
	...
	public static boolean RecordUploadProjAt(String commitMessage, String directoryName,
		...
		try (Repository repository = getRepository(directoryName)) {
			try (Git git = new Git(repository)) {
				...
				System.out.println("开始执行RecordUploadProjAt");
				System.out.println("localFileOrDirectoryName:" + localFileOrDirectoryName);
				git.add().addFilepattern(localFileOrDirectoryName).call();
				System.out.println("git.add().addFilepattern完成");
	...
	public static List<LogBean> gitLog(String directoryName) {
		try (Repository repository = getRepository(directoryName)) {
			try (Git git = new Git(repository)) {
				...
				for (RevCommit rev : logs) {
					LogBean logBean = new LogBean(rev.getAuthorIdent(),
							rev.getCommitterIdent(), rev.getName(),
							rev.getId(), rev.getCommitTime(),
							rev.getFullMessage(), rev.getShortMessage());
	...
	public static void rollback(String directoryName, String shortMessage) {
		try (Repository repository = getRepository(directoryName)) {
			try (Git git = new Git(repository)) {
				List<LogBean> logBeans = gitLog(directoryName);
				for (LogBean logBean : logBeans) {
					if (logBean.getShortMessage().equals(shortMessage)) {
						Ref ref = git.reset().setMode(ResetType.HARD)
								.setRef(logBean.getBranchNameSHA1()).call();
						break;
					}
				}
